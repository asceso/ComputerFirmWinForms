...
namespace CoreClient.StyleExtensions.Controls
{
    public class BaseStyledButton : Control
    {
        #region -- Свойства --
        ...
        private AnimationClass CurtainButtonAnim = new AnimationClass();

        //AnimationClass RippleButtonAnim = new AnimationClass();
        private AnimationClass TextSlideAnim = new AnimationClass();
        private readonly Dictionary<AnimationClass, Rectangle> RippleButtonAnimDic = new Dictionary<AnimationClass, Rectangle>();
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    KeyValuePair<AnimationClass, Rectangle> animRect = RippleButtonAnimDic.ToList()[i];
                    AnimationClass MultiRippleButtonAnim = animRect.Key;
                    ...
                    else if (MultiRippleButtonAnim.Value == MultiRippleButtonAnim.TargetValue)
                    {
                        if (MousePressed == false)
                        {
                            ...
                            MultiRippleButtonAnim.Status = AnimationClass.AnimationStatus.Completed;
                        }
                        else
                        {
                ...
                List<AnimationClass> completedRippleAnimationClasss = RippleButtonAnimDic.Keys.ToList().FindAll(x => x.Status == AnimationClass.AnimationStatus.Completed);
                for (int i = 0; i < completedRippleAnimationClasss.Count; i++)
                {
                    RippleButtonAnimDic.Remove(completedRippleAnimationClasss[i]);
                }
            }
        ...
        private void TextSlideAction()
        {
            if (MouseEntered)
            {
                TextSlideAnim = new AnimationClass("TextSlide_" + Handle, Invalidate, TextSlideAnim.Value, Width - 1);
            }
            else
            {
                TextSlideAnim = new AnimationClass("TextSlide_" + Handle, Invalidate, TextSlideAnim.Value, 0);
            }

            TextSlideAnim.StepDivider = 8;
            AnimatorClass.Request(TextSlideAnim, true);
        }

        //private void ButtonRippleAction()
        //{
        //    RippleButtonAnim = new AnimationClass("ButtonRipple_" + Handle, Invalidate, 0, Width * 2);

        //    RippleButtonAnim.StepDivider = 14;
        //    AnimatorClass.Request(RippleButtonAnim, true);
        //}

        private void ButtonMultiRippleAction()
        {
            AnimationClass MultiRippleButtonAnim = new AnimationClass("ButtonMultiRipple_" + Handle + DateTime.Now.Millisecond, Invalidate, 0, Width * 3)
            {
                StepDivider = 20
            };

            AnimatorClass.Request(MultiRippleButtonAnim);
        ...
        private void ButtonCurtainAction()
        {
            if (MouseEntered)
            {
                CurtainButtonAnim = new AnimationClass("ButtonCurtain_" + Handle, Invalidate, CurtainButtonAnim.Value, Width - 1);
            }
            else
            {
                CurtainButtonAnim = new AnimationClass("ButtonCurtain_" + Handle, Invalidate, CurtainButtonAnim.Value, 0);
            }

            CurtainButtonAnim.StepDivider = 8;
            AnimatorClass.Request(CurtainButtonAnim, true);
        }
